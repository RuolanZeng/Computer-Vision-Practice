...
cv2.imwrite(name_output+'-1.png', output1)


# program 2

output2 = np.copy(inputImage)
L_arrary = np.zeros([H2 + 1 - H1, W2 + 1 - W1, 1])
...
after_equa = histogram_equalization(L_arrary)

for i in range(H1, H2 + 1):
    for j in range(W1, W2 + 1):
        l, u, v = tmp2[i][j]
        L = int(after_equa[i][j])
        X, Y, Z = Luv_to_XYZ(L, u, v)
        R, G, B = XYZ_to_sRGB(X, Y, Z)
        output2[i][j] = B, G, R

# cv2.imshow('output1', output1)
cv2.imwrite(name_output+'-2.png', output2)
...
cv2.imwrite(name_output+'-3.png', output3)
